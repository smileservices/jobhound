...
class AbstractBowl(ABC):
    ...
    def extract_text(self, bs_result, error_id, is_list=False):
        def get_text(stuff):
            if stuff.__class__ == str:
                return stuff
            if stuff == None:
    ...
    def start(self):
        ...
        links_last_crawled = self.storage_object.filter('links_crawled', [{'term': {'status': 'ok'}}])
        links_last_errors = self.storage_object.filter('links_crawled', [{'term': {'status': 'error'}}])
        self.get_links()
